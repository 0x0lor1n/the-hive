...
return {
...

  {
    ...
    optional = true,
    opts = function(_, opts)
      -- HACK: for core `vim/*` modules the profiler builds a source path but
      -- never appends ".lua" (snacks/profiler/loc.lua:90). With vim.loader
      -- enabled (custom/init.lua) those modules report their source as a bare
      -- module name, so readfile() hits a nonexistent path and <leader>pp dies
      -- with E484. Resolve the real file; skip enrichment if there isn't one.
      -- Still present on snacks main as of 2026-08-29 -- recheck on upgrade.
      local prof_loc = require "snacks.profiler.loc"
      local ts_locs = prof_loc.ts_locs
      prof_loc.ts_locs = function(file)
