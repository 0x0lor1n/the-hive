  ...
  inputs.utilsLib.mkDisk {
    ...
    zpool.rpool = {
      ...
      datasets = {
        ...
        # (profiles/srv-workspace.nix). Fresh install: disko creates all of
        # these, no hand step (unlike penrose).
        "safe/srv" = {
          type = "zfs_fs";
          options = {
            canmount = "off";
        ...
        # Docker's image/volume store (host-elster.nix): its own dataset, not
        # an impermanence bind under /persist -- tens of GB of layers are not
        # backup material. Legacy mount + overlay2 on top; the zfs storage
        # driver is deliberately not used.
        "safe/docker" = {
          type = "zfs_fs";
