...
# shape ("called without required argument 'pkgs'"), hence this loader.
{
  ...
  # `__`-prefixed files are plain helper functions (zfs-fingerprint, mkzfscreds),
  # imported by the profiles that need them, not modules.
  files = lib.filterAttrs (n: t: t == "regular" && lib.hasSuffix ".nix" n && n != "default.nix" && !lib.hasPrefix "__" n) (builtins.readDir ./.);
