#!/usr/bin/env bash
# One PIN session: decrypt jarvis's vpn-{owt,t,w}.nix.age + user.nix.age
...
for f in vpn-owt vpn-t vpn-w user; do
...
nix-instantiate --eval --strict --raw -E "
  ...
  ''" >"$tmp/owt.conf"
nix-instantiate --eval --strict --raw -E "(import $tmp/vpn-t.nix).vpn.owt-ovpn" >"$tmp/t.ovpn"
nix-instantiate --eval --strict --raw -E "(import $tmp/vpn-w.nix).vpn.w-ovpn" >"$tmp/w.ovpn"
for n in t w; do
...
echo ">> split OK: $(wc -c <"$tmp/owt.conf")B owt.conf, $(wc -c <"$tmp/t.ovpn")B t.ovpn, $(wc -c <"$tmp/w.ovpn")B w.ovpn, 2 auth files" >&2

mkdir -p "$root/secrets/vpn" "$root/secrets/rekeyed/penrose"
pubhash=$(printf '%s' "$hostpub" | sha256sum | cut -d' ' -f1)
for name in owt.conf t.ovpn t-auth w.ovpn w-auth; do
