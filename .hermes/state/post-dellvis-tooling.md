...
- [x] ~/.claude/settings.json: env.ANTHROPIC_BASE_URL + hooks as home-manager json —
      2026-09-15, workstation/home/dev/agents.nix (home.file, HM release-25.05 has no
      programs.claude-code). DISABLE_NON_ESSENTIAL_MODEL_CALLS=1 kept.
...
- [x] ~/.config/rtk/config.toml via home-manager — 2026-09-15, agents.nix
      (exclude_commands nix/nixos-rebuild/colmena/nom/just, tee failures);
      RTK_TELEMETRY_DISABLED=1 is system-wide in agent-proxy.nix.
- [x] `rtk init -g` replaced by home.file: the PreToolUse hook is in the declared
      settings.json, ~/.claude/RTK.md is generated by the pinned rtk in a sandbox
      (rtk init writes RTK.md + `@RTK.md` into CLAUDE.md; CLAUDE.md stays the user's —
      add the line by hand). `rtk init --agent hermes` was already the
      hermes-seed-proxy user unit (agent-proxy.nix). Checked: rtk init -g only
      touches ~/.claude/{RTK.md,CLAUDE.md,settings.json} + ~/.config/rtk/filters.toml.
