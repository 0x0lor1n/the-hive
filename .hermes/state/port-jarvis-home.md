...
- [x] git identities — DONE 2026-09-14 (step 4). secrets/user-{local,entra}.nix.age now carry
      ...
      (`sec.git.includes or []`, `sec.ssh.matchBlocks or {}`). Local: client-w block on top of the
      public default author; entra: client-t.tld, azure-owt, gh-work blocks, no default. Verified in
...
- [x] ssh identities — DONE 2026-09-14 (step 4). secrets/ssh/<name>{,.pub}.age (7 keys, 14
      ...
      account. Names: ssh-github, ssh-w, ssh-wgl (local); ssh-azure-owt,
      ssh-github-owt, ssh-tgl, ssh-tprod (entra). Real aliases replace the
...
- [x] security/* — 2026-09-15 DONE.
      ...
      vpn -> profiles/vpn.nix, SYSTEM units (wg-quick-owt, openvpn-{t,w}), credentials as
      agenix runtime secrets secrets/vpn/{owt.conf,t.ovpn,t-auth,w.ovpn,w-auth}.age
      ...
      `vpn up owt` as the Entra user (no password prompt), `vpn up w` as local.
...
- [~] verify on penrose as the Entra user (daily driver) and the local user. Checks run
      ...
      - [x] git identities: Entra 3 includeIf (t, azure-owt, gh-work) -> 3 distinct
            emails via hasconfig:remote.*.url, no default identity ("OK: no default identity").
            local: git.client-e.tld (client-w) + github -> 0xolorin. gh-work ssh auth OK
      ...
      - [x] vpn: `vpn up owt` as Entra -> active, tun 192.168.4.31/32, no prompt. `vpn up
            w` as local -> active but polkit ASKED A PASSWORD (wheel = auth_admin_keep on
            manage-units). FIXED: explicit polkit YES for wheel on openvpn-w.service only.
