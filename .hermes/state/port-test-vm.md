...
cells/workstation/disks/sevastopol.nix     <- diskoConfigurations.nix sevastopol target via utilsLib.mkDisk
cells/workstation/nixosConfigurations.nix  mkHost sevastopol = foundation(common) + zfs + vmGuest + zfsNative; disko.imageBuilder.pkgs vmTools kernelImage=bzImage workaround (new nixpkgs vmTools vs disko aggregateModules)
...
secrets/generated/sevastopol/zfs-rpool-passphrase.age + secrets/rekeyed/sevastopol/7322449f...age
...
cells/workstation/profiles/secrets.nix: + age.secrets.user-ssh-key generator (.pub committed); secrets/generated/sevastopol/user-ssh-key.{age,pub} + rekeyed/sevastopol/fd7d82c6...age
cells/common: globals-options + isVm/hasTpm/userName/homeDir/hashedPassword per host, user.uid; globals.nix hosts.sevastopol (public throwaway hashes), userName/homeDir resolution
...
## verified
1-eval: `nix eval --raw .#nixosConfigurations.sevastopol.config.system.build.toplevel.drvPath` -> /nix/store/0pziad3hpwhw81fqbp8wgiq3kiz5ab8y-nixos-system-sevastopol-26.11pre-git.drv; zfsUnlock.devices = {rpool={pcrBank=sha256}}; kernel 6.18.48 (stock)
1-build: `nix build .#nixosConfigurations.sevastopol.config.system.build.toplevel` -> /nix/store/vh817lvmhz346j77srp7fxs8y936vr88-nixos-system-sevastopol-26.11pre-git; `...diskoImages` -> /nix/store/8sic0flskqmk9q8r38q6nvl0x0vzb8gy-sevastopol-disko-images (main.raw 20G); `ws-image` -> .ren/vm/sevastopol.raw
...
2-eval: toplevel drv /nix/store/6snfbx3v78h5vadp6bbxs4jrfq9w6l29-nixos-system-sevastopol-26.11pre-git.drv; zfsUnlock.passphraseFile=/run/agenix/zfs-rpool-passphrase; agenix-rekey.x86_64-linux = [edit-view generate rekey update-masterkeys]
2-secrets: `agenix generate` -> secrets/generated/sevastopol/zfs-rpool-passphrase.age (git add before rekey or "rekeyFile doesn't exist"); `agenix rekey` -> rekeyed/sevastopol/7322449f...; host pubkey captured via ssh-keyscan = ...IJEkHLFGCFphGyc0GGyxCENyE/762o1ZPOVa1Ar15ee5
...
- Scope: port ONLY the sevastopol host (native zfs enc + pcr15). Do not port luksTpm / hardware-luks-tpm.nix.
...
- activate.sh 559pw1mz…-sevastopol-26.11pre-git → RC=0, /run/current-system points to it.
...
- vmuser ssh -p 2222 works (key: `agenix view secrets/generated/sevastopol/user-ssh-key.age` — file path, no --host).
...
- sevastopol toplevel builds: /nix/store/8580z4cp0xxrx4crb5ahkizikx2k05ly-nixos-system-sevastopol-26.11pre-git
