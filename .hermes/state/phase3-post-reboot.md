...
mkdir -p /tmp/gt && cd /tmp/gt && git init -q \
  ...
  && git remote set-url origin git@gitlab.internal.tiko.ch:a/b.git && echo "tiko       -> $(git config user.email)"
cd ~ && rm -rf /tmp/gt
...
vpn up wrs && sleep 5 && vpn status && ip -br a | grep -E 'tun'
vpn down wrs; vpn status
```
